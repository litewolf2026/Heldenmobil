...
def replace_block(start_marker,end_marker,new_block,label):
    ...
    b=t.find(end_marker,a+len(start_marker)) if a>=0 else -1
    if a<0 or b<0:
        raise SystemExit(f'missing block: {label}')
    t=t[:a]+new_block+t[b:]

must_replace('HeldenMobil – HLD PoC v17.3.3','HeldenMobil – HLD PoC v18.0','title')
must_replace('const COMP_SCHEMA=6;','const COMP_SCHEMA=7;','schema')
must_replace('for(const v of [5,4,3,2,1])','for(const v of [6,5,4,3,2,1])','migration versions')
must_replace("localStorage.removeItem(companionStorageKey(5));","localStorage.removeItem(companionStorageKey(6));localStorage.removeItem(companionStorageKey(5));",'reset v6')
must_replace('HeldenMobil Qualitätsbericht v17.3.3','HeldenMobil Qualitätsbericht v18.0','audit text')
must_replace("lastQualityAudit={version:'17.3.3'","lastQualityAudit={version:'18.0'",'audit json')
must_replace('<option>Handelsgut</option></select>','<option>Handelsgut</option><option>Magischer Gegenstand</option></select>','item type')
...
pos=t.find(marker)
if pos<0: raise SystemExit('missing normalizeCompanion')
t=t[:pos]+helpers+t[pos:]
must_replace('x.inventory.items=Array.isArray(x.inventory.items)?x.inventory.items:[];','x.inventory.items=Array.isArray(x.inventory.items)?x.inventory.items.map(normalizeInventoryMagic):[];','normalize inventory magic')

artifact_card="""      <div id="artifactCard" class="comp-card artifact-card">
...
"""
replace_block('  function inventoryItemHtml(it){','\n  const MONEY_FACTOR',inventory_magic,'inventory magic functions')

must_replace("row.querySelector('.qty-plus').onclick=()=>changeQty(id,1);row.querySelector('.delete-item').onclick=()=>{", "row.querySelector('.qty-plus').onclick=()=>changeQty(id,1);row.querySelector('.magic-item-open')?.addEventListener('click',()=>{if($('#artifactItem'))$('#artifactItem').value=id;$('#artifactCard')?.scrollIntoView({behavior:'smooth',block:'start'});});row.querySelector('.delete-item').onclick=()=>{",'inventory magic shortcut')
must_replace("setupInventoryDragDrop();}","setupInventoryDragDrop();renderArtifacts();}",'render artifacts',1)
...
dashboard=r"""  function renderDashboardMagic(){
...
"""
replace_block('  function renderDashboardMagic(){','\n  function renderDashboard(){',dashboard,'dashboard magic')

must_replace('<div><strong>Zauberspeicher:</strong> ${d.magic.staffSlots.length} Slots</div><div><strong>Zuletzt lokal gespeichert:</strong>','<div><strong>Zauberspeicher:</strong> ${d.magic.staffSlots.length} Slots</div><div><strong>Magische Gegenstände:</strong> ${artifactItems().length} · ${artifactEffectCount()} Wirkungen</div><div><strong>Zuletzt lokal gespeichert:</strong>','data meta')

p.write_text(t,encoding='utf-8')
print('v18.0 robust magical-items patch applied')
